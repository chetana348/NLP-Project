...
for i in [6]: #[1, 2, 3, 4, 5, 6, 8, 10, 12]:
    for j in range(4, i+1):
        ...
        for k in range(num_trials):
            ...
            command = f"""{prg_path} --color --model "{modelpath}" -t 7 -b 24 -c 800 -n -1 --temp 0 -ngl 1 -p $'{prompt}' > {filename}"""
            ...
            print("Output:", output)
            print("Error:", error)
            with open(f"used_pairs_{i}_{j}.txt", "a") as f_up:
                f_up.write(f"{num1} {num2}\n")
