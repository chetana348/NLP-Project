...
class Model(pl.LightningModule):
    ...
    def train_dataloader(self):
        dataset = data.ExpressionDataset(100000)
    ...
    def val_dataloader(self):
        dataset = data.ExpressionDataset(1000, 100000)
        loader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True)
        return loader
    ...
    def test_step(self, batch, batch_idx):
        ...
        for (a, b), y in batch:
            ...
            answer = data.get_multiplication_answer(result)
            total += 1
            if answer == y:
                correct += 1
    ...
    def test_dataloader(self):
        dataset = data.TestDataset(1000, 200000)
        loader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True)
        return loader
    
    @torch.no_grad()
    def generate(self, data, max_length):
        self.eval()
        curi = 0
        past = None
        while len(data) < max_length:
            cur = torch.tensor([bytearray(data[curi:], 'ascii')], dtype=torch.long, device=self.device)
